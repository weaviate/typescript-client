...
describe('paths', () => {
  ...
  it('builds object delete', () => {
    return objectsPathBuilder
      ...
      .then((path) =>
        expect(path).toEqual('/objects/SomeClass/123456?consistency_level=ALL')
      )
      .catch((e) => fail(`unexpected error: ${e}`));
  });
  ...
  it('builds object merge', () => {
    return objectsPathBuilder
      ...
      .then((path) =>
        expect(path).toEqual(
          '/objects/SomeClass/123456?consistency_level=QUORUM'
        )
      )
      .catch((e) => fail(`unexpected error: ${e}`));
  });
  ...
  it('builds object update', () => {
    return objectsPathBuilder
      ...
      .then((path) =>
        expect(path).toEqual('/objects/SomeClass/123456?consistency_level=ONE')
      )
      .catch((e) => fail(`unexpected error: ${e}`));
  });
  ...
  it('builds references', () => {
    return refsPathBuilder
      ...
      .then((path) =>
        expect(path).toEqual(
          '/objects/SomeClass/123456/references/SomeProp?consistency_level=ALL'
        )
      )
      .catch((e) => fail(`unexpected error: ${e}`));
  });
