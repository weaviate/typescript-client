...
class Search {
  private static queryProperties = <T>(
    ...
    const objectProperties = properties?.filter((property) => typeof property === 'object') as unknown as
      | QueryNested<T>[]
      | undefined;
      ...
      ? GenerativeSearch.fromPartial({
          single: opts?.singlePrompt
            ? GenerativeSearch_Single.fromPartial({
                ...
                queries: opts.config
                  ? [await Serialize.generativeQuery(opts.config, toBase64FromMedia, singleOpts)]
                  : undefined,
              })
            : undefined,
          grouped: opts?.groupedTask
            ? GenerativeSearch_Grouped.fromPartial({
                task: groupedTask,
                queries: opts.config
                  ? [await Serialize.generativeQuery(opts.config, toBase64FromMedia, groupedOpts)]
                  : undefined,
  ...
  public static batchObjects = <T>(
    ...
    const iterate = (index: number) => {
      ...
      if (obj.vectors !== undefined && !Array.isArray(obj.vectors)) {
        ...
        vectorBytes = Serialize.vectorToBytes(obj.vectors as any);
      }
