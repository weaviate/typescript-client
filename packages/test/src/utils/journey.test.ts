import { DbVersionProvider, DbVersionSupport } from '@weaviate/core/utils/dbVersion';
  ...
  it('should return new version after refresh', async () => {
    ...
    const versionGetter = () => {
      ...
      switch (callsCounter) {
        ...
        default:
          throw new Error('should not be called more then 2 times');
      }
    };
    const dbVersionProvider = new DbVersionProvider(versionGetter);
    ...
    const versionGetter = () => {
      ...
      switch (callsCounter) {
        case 1:
          return Promise.resolve(VERSION_1);
        default:
          throw new Error('should not be called more then 1 time');
      }
    };
    const dbVersionProvider = new DbVersionProvider(versionGetter);
