...
requireAtLeast(1, 32, 5).describe('Integration testing of the OIDC groups', () => {
  ...
  it('should get group assignments', async () => {
    ...
    await client.roles.delete(roleName).catch((e: any) => {});
    await client.roles.create(roleName, []).catch((e: any) => {});
  ...
  it('cleanup', async () => {
    await makeClient().then((c) => {
      c.groups.oidc.revokeRoles('./assign-group', ['viewer', 'admin']).catch((e: any) => {});
      c.groups.oidc.revokeRoles('./group-1', 'viewer').catch((e: any) => {});
      c.groups.oidc.revokeRoles('./group-2', 'viewer').catch((e: any) => {});
    });
  });
});
